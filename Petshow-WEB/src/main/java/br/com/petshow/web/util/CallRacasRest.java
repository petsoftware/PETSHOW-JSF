...
public class CallRacasRest extends RestUtilCall<Racas> {
	
	public  List<Racas> getListRacasPorTipoDeAnimal(EnumTipoAnimal tipoAnimal) throws ExceptionErroCallRest, ExceptionValidation{
		client = new ResteasyClientBuilder().build();
		target= client.target(URL_BASE+"raca/consulta/por/tipo/"+tipoAnimal);
		Object entidades = null;
		try{
			entidades =  target.request().get(new javax.ws.rs.core.GenericType<List<Racas>>() {});
		}catch(Exception ex){
